...
pub fn Entities() -> Element {
    rsx! { EntitiesView { selected_id: None } }
}

#[allow(non_snake_case)]
#[component]
pub fn EntityDetail(entity_id: Uuid) -> Element {
    rsx! { EntitiesView { selected_id: Some(entity_id) } }
}

#[allow(non_snake_case)]
#[component]
fn EntityDetailPane(entity_id: Uuid) -> Element {
    let selected_entity: Resource<Result<Entity, ApiError>> =
...
#[allow(non_snake_case)]
#[component]
fn EntitiesView(selected_id: Option<Uuid>) -> Element {
    ...
    let entities: Resource<Result<Page<Entity>, ApiError>> = use_resource(move || async move {
        ...
        let off = *offset.read();
        match pf {
            None => api::fetch_entities(off, PAGE_SIZE).await,
            Some(pid) => api::fetch_project_entities(pid, off, PAGE_SIZE).await,
        }
    });
    ...
    rsx! {
        div { class: "split-view",
            ...
            if let Some(eid) = selected_id {
                EntityDetailPane { key: "{eid}", entity_id: eid }
            }
        }
    }
}
