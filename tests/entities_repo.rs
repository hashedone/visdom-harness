...
use visdom_harness::{db, entities::{self, EntityType}, projects};
...
async fn entity_create_and_get_each_type() {
    ...
    let pool = db::connect_and_migrate(&tempfile_db_url(&db_file)).await.unwrap();

    let project = projects::create(&pool, "entity test project", "desc").await.unwrap();
    ...
    assert_eq!(fetched_summary.contributing_entity_ids, vec![raw.id.clone(), knowledge.id.clone()]);
}
...
async fn entity_get_unknown_returns_none() {
    ...
    let result = entities::get(&pool, "00000000-0000-0000-0000-000000000000").await.unwrap();
...
async fn entity_list_by_project_returns_most_recent_first() {
    ...
    let raw = entities::create(&pool, &project.id, EntityType::Raw, json!({"source": "raw text"}), vec![]).await.unwrap();
    let knowledge = entities::create(&pool, &project.id, EntityType::Knowledge, json!({"fact": "x"}), vec![]).await.unwrap();
    let summary = entities::create(&pool, &project.id, EntityType::Summary, json!({"summary": "y"}), vec![raw.id.clone(), knowledge.id.clone()]).await.unwrap();

    let list = entities::list_by_project(&pool, &project.id, 10).await.unwrap();
