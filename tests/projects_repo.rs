...
async fn project_create_and_get_round_trip() {
    ...
    let created = projects::create(&pool, "test project", "a description").await.unwrap();
...
async fn project_get_unknown_returns_none() {
    ...
    let result = projects::get(&pool, "00000000-0000-0000-0000-000000000000").await.unwrap();
...
async fn project_exists_true_and_false() {
    ...
    let created = projects::create(&pool, "exists test", "desc").await.unwrap();

    assert!(projects::exists(&pool, &created.id).await.unwrap());
    assert!(!projects::exists(&pool, "00000000-0000-0000-0000-000000000000").await.unwrap());
}
